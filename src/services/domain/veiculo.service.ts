...
export class VeiculoService {
  ...
  findVeiculosCustomPage(page: number = 0, linesPerPage: number = 24, orderBy: string = 'ASC'): Observable<VeiculoDTO[]> {
    ...
    let url = `${API_CONFIG.baseUrl}/veiculos/buscar/avancada?page=${page}&linesPerPage=${linesPerPage}&direction=${orderBy}`;

    if (pesquisaVeiculo.marca != null) {
      params.set('marca', pesquisaVeiculo.marca);
    }

    if (pesquisaVeiculo.modelo != null) {
      params.set('modelo', pesquisaVeiculo.modelo);
    }
